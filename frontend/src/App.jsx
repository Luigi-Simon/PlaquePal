...
  const [isConnected, setIsConnected] = useState(false);
  ...
  // NEW: Triggers the automatic report display
...
          setIsConnected(true);
          setShowReport(false); // Reset report on new connection
        };
...
            if (data.state) {
              setState(data.state);
              // NEW: If backend confirms success, show the report card!
              if (data.state === 'SUCCESS') {
                setShowReport(true);
              } else if (showReport) {
                // If the backend moves out of SUCCESS, hide report
                setShowReport(false);
              }
            }
            
...
          setIsConnected(false);
          setShowReport(false); // Reset report on disconnect
          
...
  }, [showReport]);
  ...
  // Handle restarting the scan loop from the frontend
  const handleNewScan = () => {
    // If connected, we should ideally send a reset command to backend
    if (isConnected && wsStatus.includes('Established')) {
      // Create a dummy WebSocket instance to send message (temporary solution)
      const wsTemp = new WebSocket('ws://localhost:8000/ws');
      wsTemp.onopen = () => {
        wsTemp.send(JSON.stringify({ command: 'RESET_SCAN' }));
        wsTemp.close();
      };
    }
    // Visually reset frontend immediately
    setState('POSITIONING');
    setHoldProgress(0);
    setShowReport(false);
  };

  // Color based on state (Only used for Sidebar now)
...
          <div className="absolute inset-0 bg-[repeating-linear-gradient(0deg,transparent,transparent_2px,rgba(0,0,0,0.3)_2px,rgba(0,0,0,0.3)_4px)] pointer-events-none opacity-50"></div>
          ...
          {/* Regular Scanning UI (Only show when NOT success report) */}
          {!showReport && (
            <>
              {/* STATE Indicator REMOVED from top-left */}

              {/* ANGLE Indicator - Repositioned and stylized */}
              ...
              {/* Main Feedback Loop - MASSIVELY ENHANCED */}
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center w-[90%] z-10">
                ...
                {/* Visual Angle Cue */}
                {angle.latitude !== 0 && (
                   <Zap className={`mx-auto mt-8 animate-pulse ${angle.latitude > 0 ? 'rotate-180 text-yellow-500' : 'text-yellow-500'}`} size={48} />
                )}
              </div>

              {/* Minor Guidance Messages */}
          ...
          {/* DYNAMIC SUCCESS REPORT CARD - Slides in automatically */}
          {showReport && (
            <div className="absolute inset-0 bg-slate-900 z-40 p-10 flex flex-col animate-in slide-in-from-bottom-6 duration-700 ease-out overflow-y-auto border-t-4 border-green-500/80">
              ...
              <div className="flex justify-between items-center mb-10 border-b border-slate-800 pb-6">
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-slate-600 mt-2 font-mono">Scan ID: PKPL-{new Date().toISOString().slice(11,19)}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-8 mb-10">
                ...
                {/* SQS Score - Composite based on telemetry */}
...
          {/* Connection Warning - Elevated Z-Index */}
          {!isConnected && (
...
        {/* Real-time Metrics Sidebar - remains relevant even with the large main text */}
